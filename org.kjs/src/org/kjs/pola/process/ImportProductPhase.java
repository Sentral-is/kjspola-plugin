...
public class ImportProductPhase extends SvrProcess implements ImportProcess
{
    ...
    protected String doIt() throws Exception {
        ...
        final HashMap<Integer, Integer> phaseHeaderByProduct = new HashMap<Integer, Integer>();
        try {
            ...
            while (rs.next()) {
                ...
                Integer headerId = phaseHeaderByProduct.get(Integer.valueOf(M_Product_ID));
                if (headerId == null) {
                    int existing = DB.getSQLValueEx(this.get_TrxName(),
                            "SELECT KJS_ProductPhase_ID FROM KJS_ProductPhase WHERE M_Product_ID=?",
                            M_Product_ID);
                    if (existing <= 0) {
                        final X_KJS_ProductPhase pp = new X_KJS_ProductPhase(this.getCtx(), 0, this.get_TrxName());
                        ...
                        existing = pp.getKJS_ProductPhase_ID();
                    }
                    headerId = Integer.valueOf(existing);
                    phaseHeaderByProduct.put(Integer.valueOf(M_Product_ID), headerId);
                }
                final X_KJS_ProductPhaseLine ppl = new X_KJS_ProductPhaseLine(this.getCtx(), 0, this.get_TrxName());
                ppl.setKJS_ProductPhase_ID(headerId.intValue());
                ...
                imp.setKJS_ProductPhase_ID(headerId.intValue());
